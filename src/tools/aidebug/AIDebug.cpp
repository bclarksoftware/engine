...
void AIDebug::dbgEntities() {
	if (ImGui::Begin("Entities")) {
		...
		if (_stateWorldMsg != nullptr) {
			ImGui::InputText("Filter", _entityListFilter, sizeof(_entityListFilter));
...
void AIDebug::dbgTreeNode_r(const ai::StateNode *node, int level) {
...
#if 0
	// TODO: editing
	// add column with icons for adding a child, deleting a child and updating a child.
	ImGui::SameLine();
	if (ImGui::Selectable(ICON_FA_EDIT)) {
	}
	ImGui::SameLine();
	if (ImGui::Selectable(ICON_FA_MINUS)) {
	}
	ImGui::SameLine();
	if (ImGui::Selectable(ICON_FA_PLUS)) {
	}
#endif

	ImGui::TableNextCell();

	ImGui::TextUnformatted(nodeParameters); ImGui::TableNextCell();
	ImGui::TextUnformatted(nodeType); ImGui::TableNextCell();
	...
	ImGui::TextUnformatted(conditionName); ImGui::TableNextCell();
	ImGui::PopStyleColor(1);
	ImGui::Text("%s", ai::EnumNameTreeNodeStatus(node->status())); ImGui::TableNextCell();
...
bool AIDebug::dbgMapIsVisible(ImVec2& pos, const ImVec2& mapMins, const ImVec2& mapMaxs) const {
...
void AIDebug::dbgMap() {
	...
	if (_centerOnSelection) {
		for (const auto &e : *_stateWorldMsg->states()) {
			...
			if (selected) {
				_dbgMapOffset = ImVec2(-e->position()->x() * _zoom + _frameBufferDimension.x / 2.0f, -e->position()->z() * _zoom + _frameBufferDimension.y / 2.0f);
				break;
			}
		}
	...
	if (ImGui::Begin("##map", nullptr, ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoSavedSettings)) {
		...
		const float radius = 10.0f * _zoom;
		const ImVec2 entSize(radius * 2.0f);
		ImDrawList *draw = ImGui::GetWindowDrawList();
		draw->PushClipRect(mapMins, mapMaxs, true);
		for (const auto &e : *_stateWorldMsg->states()) {
			const float x = e->position()->x() * _zoom;
			const float y = e->position()->z() * _zoom;
			ImVec2 entPos(_dbgMapOffset.x + x, _dbgMapOffset.y + y);
			...
			const bool hover = ImGui::TooltipText("ID: %" PRIChrId "\nStrength: %.2f/%.2f", e->character_id(),
			...
			const float viewRadius = (float)attribCurrent[core::enumVal(attrib::Type::VIEWDISTANCE)] * _zoom;
			if (viewRadius > radius) {
				draw->AddCircle(entPos, (float)viewRadius, viewRadiusColor, 18, 1.0f);
			}

			const float attackRadius = (float)attribCurrent[core::enumVal(attrib::Type::ATTACKRANGE)] * _zoom;
			if (attackRadius > 0.0) {
	...
	if (ImGui::IsWindowHovered()) {
		_zoom = core_max(0.01f, _zoom + ImGui::GetIO().MouseWheel * 0.1f);
...
void AIDebug::dbgBar() {
	...
	ImGui::SameLine();
	if (ImGui::ToggleButton("Pause", _pause)) {
		togglePause();
	}
	ImGui::SameLine();
	...
	ImGui::SameLine();
	if (ImGui::ToggleButton("Stats", _showStats)) {
		_showStats ^= true;
	}
	ImGui::SameLine();
